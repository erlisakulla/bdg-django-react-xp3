...
class gameview(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['get'])
    @swagger_auto_schema(operation_description="Returns Other Player Status if infosharing enabled",
    ...
    def getsharedinfo(self, request, pk=None):
        ...
        if game.info_sharing:
            ...
            for role in roles:
                ...
                alldetail.append(
                    {"Role": role.roleName, "OrderPlaced": role.ordered, 'PlayedBy': playedby})
            #serialize = RoleSerializer(roles, many=True)
